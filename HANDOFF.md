...
| 80.1 | Bento+Blue refresh on workspace per locked mockup: sub-tabs row moved above the line title (3 stacked bento cards: Tabs → Meta → Body), rounded-2xl with soft shadow + 1px ring on slate-50 bg, sky gradient on Total tile + sky-500→sky-600 progress bar, rails wrapped in white inner cards. | ✅ PR #82 |
| 80.2 | Close two workspace coming-soon stubs: (a) edit-line modal — extract LineForm with `compact` prop, add notes textarea, new LineEditDialog wired to LineItemDetail pencil; (b) chapter-scoped doc upload — new `uploadDocument` shared helper writes `tags=['chapter:<id>']`, used by both DocumentsPanel and ChapterDocumentRail. Drop the obsolete `editLineComingSoon` and `uploadComingSoon` keys. | ✅ PRs #83 + #84 |
| 80.3 | Workspace layout reverted to locked mockup: Meta strip stays a standalone bento card on top; sub-tabs + active body now merged into one combined bento card (tabs as top edge, single border-b, body fills below) — was 3 separate cards in 80.1. Matches `apps/web/public/mockups/workspace.html` lines 451-457 verbatim. | ✅ PR #86 |
